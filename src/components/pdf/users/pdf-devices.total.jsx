...
function PdfDevices({ data, company, branch }) {
        ...
        >
          <View style={styles.boxContent}>
            ...
            <Text style={styles.textSecond}>
              Sucursal :{" "}
              <Text style={{ fontWeight: "extrabold" }}>{branch}</Text>
            </Text>
  ...
  if (data)
    return data?.map((ValueUser, index) => (
      <View style={StyleHeader.container} key={index}>
       ...
       { ValueUser?.codigo_dispositivo}
        </Text>
        <Text style={StyleHeader.TextGnero}>{ValueUser?.nombre} </Text>
        <Text style={StyleHeader.TextSpecialization}>
          {ValueUser?.tipo === ""
            ? "----"
            : capitalizeText(ValueUser?.tipo ?? "------")}{" "}
        </Text>
        <Text style={StyleHeader.TextSpecialization}>
          {ValueUser?.marca === ""
            ? "----"
            : capitalizeText(ValueUser?.marca ?? "------")}{" "}
        </Text>
        <Text style={StyleHeader.Anydesk}>{ValueUser?.tipo_Disp} </Text>
        <Text
          style={{
...
function TableHeader() {
  ...
  return (
    <View style={StyleHeader.container}>
      ...
      <Text style={StyleHeader.TextSecond}>codigo</Text>
      <Text style={StyleHeader.TextGnero}>Nombre</Text>
      <Text style={StyleHeader.TextSpecialization}>Dispositivo</Text>
      <Text style={StyleHeader.TextSpecialization}>Marca</Text>
      <Text style={StyleHeader.Anydesk}>Tipo</Text>
      <Text style={StyleHeader.TextStatus}>Estado</Text>
      
      <Text style={StyleHeader.Email}>Fecha Fabricacion</Text>
    </View>
  );
}
