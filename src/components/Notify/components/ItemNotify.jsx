...
function ItemNotify({ Description, UserCreate, Hora, isRead, idNotify ,colorSeleccionado}) {
  ...
  return (
    ...
    >
      ...
      <span className="h-full  w-full rounded-md" style={{ backgroundColor: colorSeleccionado }}></span>
      <header>
        <span className="text-xs font-semibold bg-black/70 text-white px-2 rounded-md ">{UserCreate}</span>
        <p className="text-sm ">
          {ViewMoreText === false ? (
            <TruncateText
